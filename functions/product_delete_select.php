...
renderHeader('Select Products to Delete');

?>

<h2>Select Products to Delete</h2>

<?php include('../includes/product_actions.php'); ?>

<form method="POST" action="product_delete_select.php">
...
<form method="POST" action="../functions/product_delete_multiple.php">
    <div id="product-list">
        <?php
        ...
        if ($result->num_rows > 0) {
            ...
            while($row = $result->fetch_assoc()) {
                echo "<tr><td><input type='checkbox' name='selected_products[]' value='" . $row["id"] . "'></td><td>" . $row["name"]. "</td><td>" . $row["description"]. "</td><td>" . $row["price"]. "</td><td>" . $row["quantity"]. "</td><td>" . $row["category_name"] . "</td></tr>";
            }
            echo "</table>";
